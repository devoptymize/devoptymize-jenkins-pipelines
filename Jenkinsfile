properties([
    parameters([
        ...
        [$class: 'DynamicReferenceParameter', 
            ...
            name: 'APP_SUBNETS', // Sets the name of the parameter as "PRIVATE_SUBNETS"
        ...
        [$class: 'DynamicReferenceParameter', 
            ...
            name: 'DATA_SUBNETS', // Sets the name of the parameter as "PUBLIC_SUBNETS"
            omitValueField: true, // Hides the value field in the generated form element (since the value will be dynamically generated)
        ...
        separator(name: "EFS-PARAMETERS", sectionHeader: "EFS Parameters",			
            separatorStyle: "border-width: 0",
			sectionHeaderStyle: """
				background-color: #E1D9D1;
				text-align: center;
        ...
        [$class: 'CascadeChoiceParameter',
			...
			filterLength: 4,
			filterable: true,
			randomName: 'choice-parameter-5631321347061229',
			referencedParameters: 'ACTION',
			name: 'ENCRYPTED_BOOLEAN',
			script: [
				$class: 'GroovyScript',
				fallbackScript: [
					classpath: [],
        ...
        string(
                ...
                trim: true,
                description: '''Select the performance mode of the file system. The allowed values are generalPurpose | maxIO'''
        ),
        ...
        string(
                ...
                trim: true,
                description: '''The throughput, measured in mebibytes per second (MiBps), that you want to provision for a file system that you're creating. Required if ThroughputMode is set to provisioned. Valid values are 1-3414 MiBps'''
        ),
        ...
        string(
                ...
                trim: true,
                description: '''Specifies the throughput mode for the file system. The mode can be bursting, provisioned, or elastic. If you set ThroughputMode to provisioned, you must also set a value for ProvisionedThroughputInMibps.'''
        ),

        [$class: 'CascadeChoiceParameter', 
            choiceType: 'PT_SINGLE_SELECT', 
            description: '''The ARN of the AWS KMS key to be used to protect the encrypted file system''', 
            name: 'KMS_KEY_ID', 
            randomName: 'choice-parameter-563221518619', 
            referencedParameters: 'ACTION,CREDENTIAL,VPC_NAME,VPC_ID,REGION', 
            script: [
                $class: 'GroovyScript', 
                fallbackScript: [
                    classpath: [], 
        ...
        separator(name: "ELASTICACHE-PARAMETERS", sectionHeader: "ELASTICACHE Parameters",			
            separatorStyle: "border-width: 0",
			sectionHeaderStyle: """
				background-color: #E1D9D1;
				text-align: center;
        ...
        [$class: 'CascadeChoiceParameter', 
            ...
            randomName: 'choice-parameter-5321230789999', 
            referencedParameters: 'ACTION,CREDENTIAL,REGION', 
            script: [
                $class: 'GroovyScript',  
                fallbackScript: [
                        ...
                        if (ACTION.equals("Create") || ACTION.equals("Modify")) {
                            ...
                            for (creds in jenkinsCredentials) {
                                if (creds.id == "$CREDENTIAL") {
                                    ...
                                    def command = "env AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY aws elasticache describe-reserved-cache-nodes-offerings --query ReservedCacheNodesOfferings[].CacheNodeType --output text"
            ...
            script: [
                ...
                script: [
                    ...
                    script: 
                        ...
                        if (ACTION.equals("Create")){
                               return ['aurora-mysql','aurora-postgresql']
                        }        
...
pipeline{ 
    ...
    environment { // Defines environment variables used by the pipeline
        resource = "aws-3t-wp" // Sets the value of the "resource" variable to "network"
            ...
            steps {
                script{ // Allows for execution of arbitrary Groovy code within the pipeline
                    if (env.IAC_TOOL == "Terraform") {
                        ...
                        withAWS(region: "${env.REGION}") {
                        cft_resource_provision ('aws-3t-wp.j2') // Calls a function named "cft_resource_provision()" here we need to provide the "jinja2" file name to provision a resource (defined in the jenkins shared library)                     
