properties([
    parameters([
        ...
        [$class: 'DynamicReferenceParameter', 
            ...
            description: 'Private subnet CIDR ranges in a list format to create the private subnets.Here we can create number of subnet which is based on our requirement and the subnet will create based on the number of cidr range which we have provide ,Suggests providing a comma-separated list of CIDR ranges enclosed in square brackets, such as["10.0.2.0/24","10.0.3.0/24"]', // Describes the purpose of the parameter and provides an example input format
        ...
        [$class: 'DynamicReferenceParameter', 
            ...
            defaultValue: 'ami-0ded8326293d3201b',
        ...
        [$class: 'DynamicReferenceParameter', 
            choiceType: 'ET_FORMATTED_HTML', 
            description: '''The type of volume to attach. Example gp2 ''', 
            name: 'VOLUME_TYPE', 
            omitValueField: true,
            randomName: 'choice-parameter-500131445117869', 
            referencedParameters: 'ACTION', 
            script: [
                $class: 'GroovyScript', 
                fallbackScript: [
        ...
        [$class: 'DynamicReferenceParameter', 
                choiceType: 'ET_FORMATTED_HTML', 
                description: '''Type a name for your multi AZ DB instance. The name must be unique across all multi AZ DB instance owned by your AWS account in the current AWS Region. The DB identifier is case-insensitive, but is stored as all lowercase. Constraints: 1 to 60 alphanumeric characters or hyphens. First character must be a letter. Can't contain two consecutive hyphens. Can't end with a hyphen.''', 
                name: 'RDS_IDENTIFIER', 
                omitValueField: true,
                randomName: 'choice-parameter-56313113156179909', 
                referencedParameters: 'ACTION,CREDENTIAL,REGION', 
                script: [
                    $class: 'GroovyScript',  
                    fallbackScript: [
        ...
        [$class: 'CascadeChoiceParameter', 
                choiceType: 'PT_SINGLE_SELECT', 
                description: 'The RDS instance engine type.', 
                filterLength: 4, 
                filterable: true, 
                randomName: 'choice-parameter-56313111561789619', 
                referencedParameters: 'ACTION,RDS_IDENTIFIER,REGION,CREDENTIAL', 
                name: 'ENGINE_TYPE', 
                script: [
                    $class: 'GroovyScript', 
                    fallbackScript: [
                            ...
                            if (ACTION.equals("Modify")){
                                ...
                                for (creds in jenkinsCredentials) {
                                    if(creds.id == "$CREDENTIAL"){
                                        ...
                                        def command = "env AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY aws rds describe-db-instances --db-instance-identifier=$RDS_IDENTIFIER --query=DBInstances[].Engine  --output text --region $REGION"
                                        def proc = command.execute()
                                        proc.waitFor()              
                                        def output = proc.in.text
                                        def exitcode= proc.exitValue()
        ...
        [$class: 'CascadeChoiceParameter', 
                choiceType: 'PT_SINGLE_SELECT', 
                description: '''The engine version to use. The Multi-AZ DB Instances deployment option is available for MySQL version 8.0.28 or later and PostgreSQL version 14.4 R1 or later.''', 
                name: 'ENGINE_VERSION', 
        ...
        [$class: 'CascadeChoiceParameter', 
            ...
            randomName: 'choice-parameter-5320231889999', 
                        ...
                        if (ACTION.equals("Create") || ACTION.equals("Modify")) {
                            ...
                            for (creds in jenkinsCredentials) {
                                if(creds.id == "$CREDENTIAL"){
                                    ...
                                    def command = "env AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY  aws rds describe-orderable-db-instance-options --engine=$ENGINE_TYPE --engine-version=$ENGINE_VERSION --query OrderableDBInstanceOptions[?MultiAZCapable].DBInstanceClass --region $REGION --output text"
        ...
        separator(name: "NOTIFICATION-PARAMETERS", sectionHeader: "Notification Parameters",			
            separatorStyle: "border-width: 0",
			sectionHeaderStyle: """
				background-color: #E1D9D1;
				text-align: center;
        ...
        [$class: 'DynamicReferenceParameter', 
            choiceType: 'ET_FORMATTED_HTML', 
            description:  '''Type a mail ids for getting sns notification ["abcd5432@gmail.com", "xyz9879@gmail.com"]''',
            name: 'EMAIL_ADDRESS', 
            omitValueField: true,
            randomName: 'choice-parameter-563000145117869', 
            referencedParameters: 'ACTION', 
            script: [
                $class: 'GroovyScript', 
                fallbackScript: [
...
pipeline{ 
    ...
    environment { // Defines environment variables used by the pipeline
        resource = "aws-rds-stack" // Sets the value of the "resource" variable to "network"
            ...
            steps {
                script{ // Allows for execution of arbitrary Groovy code within the pipeline
                    if (env.IAC_TOOL == "Terraform") {
                        ...
                        withAWS(region: "${env.REGION}") {
                        cft_resource_provision ('aws-rds-stack.j2') // Calls a function named "cft_resource_provision()" here we need to provide the "jinja2" file name to provision a resource (defined in the jenkins shared library)                     
