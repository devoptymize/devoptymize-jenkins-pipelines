...
| `INSTANCE_TYPE` | This parameter specifies the type of Amazon EC2 instance that you want to launch.                                        | `r6a.2xlarge` |
| `KEY_PAIR`  |  This parameter is used for specifying the key pair that will be associated with the EC2 instance. It consists of a public key and a private key. When you launch an EC2 instance, you can use a key pair to securely access the instance via SSH (Secure Shell) or other remote access methods              | `odoo` |
...
| ` OPERATINGSYSTEM ` | This parameter specifies the type of operating system . For terraform u dont need to choose anything. Only for the cft we need to choose between linux and windows.                                  | ` Dont have to choose OS` |
...
| ` PORT ` | parameter likely specifies a network port number that will be used for communication between your application or service and other entities.
| ` PROTOCOL` | This parameter indicates the communication protocol that will be used for data exchange over the specified port                         | ` HTTPS` |
...
| ` ENGINE_TYPE `        |  This parameter refers to the type of database engine that the RDS cluster will use. In this case, the value aurora-mysql indicates that the RDS cluster will use Amazon Aurora, a fully managed MySQL-compatible relational database engine          | ` aurora-mysql` |
| ` ENGINE_VERSION ` |   This parameter specifies the specific version of the database engine to be used.              | ` 5.7.mysql-aurora.2.07.9` |
...
| ` DB_INSTANCE_CLASS ` |  This parameter defines the compute and memory resources allocated to individual database instances within the RDS cluster.                        | `db.m6g.16xlarge `|
